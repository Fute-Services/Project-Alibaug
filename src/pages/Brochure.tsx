...
import { loadBrochure, pdfFile, type VirtualPage } from "../lib/brochurePdf";
import bgImage from "../assets/homepagefinal.webp";
...
const PageRenderer = ({
    ...
    if (page.pdfPageNumber === -1) {
        return (
            <div className="w-full h-full bg-[#1c1e16] border border-[#2a2d1e] flex flex-col items-center justify-center p-8 select-none text-center">
                <div className="w-16 h-16 mx-auto mb-4 border border-[#DB9B2F]/40 rounded-full flex items-center justify-center">
                    <span className="text-[#DB9B2F] font-serif text-2xl font-bold">H</span>
                </div>
                <h3 className="text-[#FFE2A4] font-serif text-lg tracking-widest uppercase mb-2">
                    Hiranandani Sands
                </h3>
                <div className="w-12 h-[1px] bg-[#DB9B2F]/50 mx-auto my-3" />
                <p className="text-[#FFE2A4]/60 text-[10px] tracking-widest uppercase font-sans">
                    Premium Coastal Plots
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="w-full h-full bg-[#1b1c18] relative flex items-center justify-center">
            {renderState === "error" && (
...
export default function Brochure() {
    ...
    // Load PDF via the shared cached loader (already preloaded in the background on app start)
    useEffect(() => {
        let isMounted = true;

        loadBrochure()
            .then(({ pdf, virtualPages: pagesList }) => {
                if (isMounted) {
                    setPdfDoc(pdf);
            ...
            <div className="w-full h-full z-10 pl-[max(12%,64px)] py-2 pr-2 sm:py-4 sm:pr-4 lg:py-6 lg:pr-6 flex items-center justify-center relative">
                <div
                    ...
                    style={{
                        ...
                        boxShadow: "0 20px 50px rgba(0, 0, 0, 0.7), 0 0 30px rgba(219, 156, 47, 0.08)"
                    }}
                >
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
                            ...
                            <div className="flex items-center gap-0.5 sm:gap-1 border border-[#DB9B2F]/20 rounded-full px-1.5 sm:px-2 py-0.5 bg-black/30">
                                <button
                                    ...
                                    className="p-1 hover:text-[#FFE2A4] text-white/60 disabled:opacity-30 disabled:pointer-events-none transition-colors"
                                    title="Zoom Out"
                                >
                                    <ZoomOut size={13} />
                                </button>
                                ...
                                {zoomScale > 1.0 && (
                                    <button
                                        onClick={resetZoom}
                                        className="p-1 hover:text-[#FFE2A4] text-white/60 transition-colors ml-0.5 border-l border-[#DB9B2F]/20"
                                        title="Reset Zoom"
                                    >
                                        <RotateCcw size={11} />
                                    </button>
                    ...
                    <div
                        className="flex-1 w-full h-full bg-[#1b1c18] overflow-hidden relative select-none"
                        onWheel={handleWheel}
                    ...
                    >
                        {loading && (
                            <div className="w-full h-full flex flex-col items-center justify-center gap-3">
                                <Loader2 className="w-10 h-10 animate-spin text-[#DB9B2F]" />
                                <p className="text-[#FFE2A4] font-sans text-xs tracking-widest uppercase animate-pulse">
                                    Loading Brochure...
                                </p>
                            </div>
                        )}
                        ...
                        {error && (
                            <div className="w-full h-full flex flex-col items-center justify-center text-center px-4">
                                ...
                                <a
                                    href={pdfFile}
                                    download="Hiranandani_Sands_Brochure.pdf"
                                    className="px-6 py-2 rounded-full bg-[#DB9B2F] text-slate-950 font-bold text-xs tracking-wider uppercase transition-all duration-300 shadow-md"
                                >
                                    Download to View
                                </a>
                            </div>
                                ...
                                {isEditingPage ? (
                                    <div className="absolute bottom-2 sm:bottom-6 left-1/2 -translate-x-1/2 px-3 sm:px-5 py-1.5 sm:py-2 rounded-full bg-black/85 border border-[#DB9B2F] backdrop-blur-md text-[#FFE2A4] text-[10px] sm:text-[11px] tracking-widest font-sans font-bold shadow-lg z-30 flex items-center gap-2 whitespace-nowrap">
                                        ...
                                        <input
                                            ...
                                            className="w-11 bg-black/45 border border-[#DB9B2F]/50 rounded-md py-0.5 text-[#FFE2A4] focus:outline-none focus:border-[#DB9B2F] text-center font-bold text-[11px]"
                                        />
                                        <span className="text-[#FFE2A4]/60">/ {pdfDoc?.numPages}</span>
                                    </div>
                                ) : (
                                    <div
                                        onClick={() => {
